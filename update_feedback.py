...
heading=Label(t,text='Update Feedback', font=('helvetica',14))
...
def updatefeedback():
    ...
    sql = "update feedback set  callid='%s', engid='%s', rating='%s' where callno ='%s'" %(xb,xc,xd,xa)
    
    cur.execute(sql)
    db.commit()
    
    messagebox.showinfo('Hi','Feedback Updated')
...
"""
update=Button(t,text='Update',command=updatefeedback)
