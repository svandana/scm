...
heading=Label(t,text='Delete Customer', font=('helvetica',14))
...
def deletecustomer():
    ...
    xa = customeridCb.get()
    sql = "delete from customer where customerid ='%s'" %(xa)
    
    cur.execute(sql)
    db.commit()
    
    messagebox.showinfo('Hi','Customer Deleted')
...
delete=Button(t,text='Delete',command=deletecustomer)
