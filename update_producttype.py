...
def updateproducttype():
    ...
    sql = "update producttype set typename='%s',  description='%s' where producttypeid ='%s'" %(xb,xc,xa)
    
    cur.execute(sql)
    db.commit()
    
    messagebox.showinfo('Hi','Product Type Updated')
...
update=Button(t,text='Update',command=updateproducttype)
