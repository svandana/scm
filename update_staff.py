...
heading=Label(t,text='Update Staff', font=('helvetica',14))
...
def updatestaff():
    ...
    sql = "update staff set sname='%s', address='%s', phone='%s', email='%s' where staffid ='%s'" %(xb,xc,xd,xe,xa)
    
    cur.execute(sql)
    db.commit()
    
    messagebox.showinfo('Hi','Staff Updated')
...
update=Button(t,text='Update',command=updatestaff)
