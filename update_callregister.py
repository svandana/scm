...
heading=Label(t,text='Update - Registered Call', font=('helvetica',14))
...
def updatecallregister():
    ...
    sql = "update callregister set callid='%s', productid='%s', serviceid='%s', dateofcall='%s',solvedate='%s' where callno ='%s'" %(xb,xc,xd,xe,xf,xa)
    
    cur.execute(sql)
    db.commit()
    
    messagebox.showinfo('Hi','Call Register Updated')
...
update=Button(t,text='Update',command=updatecallregister)
