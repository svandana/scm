...
def updateengineer():
    ...
    sql = "update engineer set ename='%s', address='%s', phone='%s', email='%s',producttypeid ='%s' where engid ='%s'" %(xb,xc,xd,xe,xf,xa)
    
    cur.execute(sql)
    db.commit()
    
    messagebox.showinfo('Hi','Engineer Updated')
...
update=Button(t,text='Update',command=updateengineer)
