...
heading=Label(t,text='Update Service Type', font=('helvetica',14))
...
def updateservice():
    ...
    xa = serviceIdCb.get()
    xb = productIdEntry.get()
    xc = prodTyIdEntry.get()
    xd = snameEntry.get()
    xe = int(costEntry.get())
    xf = daysSolveEntry.get()
    
    sql = "update servicetype set productid='%s', producttypeid='%s', sname='%s', cost=%d,daystosolve='%s' where serviceid ='%s'" %(xb,xc,xd,xe,xf,xa)
    
    cur.execute(sql)
    db.commit()
    
    messagebox.showinfo('Hi','Service Type Updated')
...
update=Button(t,text='Update',command=updateservice)
